...
export const loginController = async (req, res) => {
  ...
  try {
    ...
    const userDetails = await User.findOne({ mobile: req.body.mobile }).select(
      "+password"
    );
    ...
    const isAdminUser = !!(await Admin.findOne({
...
export const verifyOTPController = async (req, res) => {
  ...
  return res.status(400).json({
    success: false,
    message: "OTP verification is deprecated. Use password-based login.",
  });
};
